...
window.onload = function() {
    ...
    if (canvas.getContext) {
        ...
        function translate() {
            ...
            ctx.fillText("illustration:Riiko Fujiwara", x - 250, y - 20);
            for (let i = 0; i < N; i++){
                pos_X[i] = pos_X1[i];
                pos_Y[i] = pos_Y1[i];
                vel_X[i] = vel_X1[i];
        ...
        function cohesion(j){
            ...
            let num = 0;
            for (let i = 0; i < N; i++){
                if((pos_X[j]-pos_X[i])**2 + (pos_Y[j]-pos_Y[i])**2 < r**2 && i != j && tag[i] == tag[j]){//距離判定
                    num ++;
                    acc1[0] += pos_X[i];
                    acc1[1] += pos_Y[i];
                }
                // if((pos_X[j]-pos_X[i])**2 + (pos_Y[j]-pos_Y[i])**2 < r**2 && i != j && tag[i] != tag[j]){
                //     num ++;
                //     acc1[0] -= pos_X[i];
                //     acc1[1] -= pos_Y[i];
                // }
        ...
        function separation(j){
                ...
                for (let i = 0; i < N; i++){
                    if((pos_X[j]-pos_X[i])**2 + (pos_Y[j]-pos_Y[i])**2 < r**2 && i != j && tag[i] == tag[j]){//距離判定
                        num ++;
                        acc2[0] += (pos_X[j] - pos_X[i])/((pos_X[j]-pos_X[i])**2 + (pos_Y[j]-pos_Y[i])**2)**2;
                        acc2[1] += (pos_Y[j] - pos_Y[i])/((pos_X[j]-pos_X[i])**2 + (pos_Y[j]-pos_Y[i])**2)**2;
                    }
                    // if((pos_X[j]-pos_X[i])**2 + (pos_Y[j]-pos_Y[i])**2 < r**2 && i != j && tag[i] != tag[j]){//距離判定
                    //     num ++;
                    //     acc2[0] -= (pos_X[j] - pos_X[i])/((pos_X[j]-pos_X[i])**2 + (pos_Y[j]-pos_Y[i])**2)**2;
